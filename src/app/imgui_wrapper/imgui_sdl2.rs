...
impl imgui::ClipboardBackend for Sdl2ClipboardBackend {
    fn get(&mut self) -> Option<String> {
        self.0.clipboard_text().map_or(None, |s| Some(s))
    }

    fn set(&mut self, value: &str) {
        match self.0.set_clipboard_text(value) {
            Ok(_) => {},
            Err(err) => {
                // @TODO logger
                println!("[imgui_wrapper][clipboard] Could not set clipboard: {err}");
            }
        }
    }
}
...
impl ImguiSdl2 {
    pub fn new(
        ...
        imgui.io_mut().key_map[Key::Insert as usize] = Scancode::Insert as u32;
        ...
        imgui.io_mut().key_map[Key::KeyPadEnter as usize] = Scancode::KpEnter as u32;
    ...
    pub fn prepare_frame(
        ...
        // Update delta time
        let counter   = sdl.timer().unwrap().performance_counter();
        let frequency = sdl.timer().unwrap().performance_frequency();
        let delta = if self.last_time > 0 {
            (counter - self.last_time) as f32 / frequency as f32
        } else {
            1.0/60.0
        };
